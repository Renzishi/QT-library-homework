...
static unsigned int tempA = A, tempB = B, tempC = C, tempD = D, strlength = 0;
static string format(unsigned int num);
static void iterateFunc(unsigned int* X);
static vector<unsigned int> padding(string src);
    // F函数
...
#define NORMAL "^[A-Za-z0-9]+$"

static void inline setLimit(QString s, QLineEdit *t) {
    QRegExp regx(s);
